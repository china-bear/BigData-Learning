...
package edu.bear.flink.examples.java.distcp;
...
public class DistCp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<edu.bear.flink.examples.java.distcp.FileCopyTask> tasks = getCopyTasks(sourcePath);
        LOGGER.info(
                "Copy task initialization took " + (System.currentTimeMillis() - startTime) + "ms");

        DataSet<edu.bear.flink.examples.java.distcp.FileCopyTask> inputTasks =
                new DataSource<>(
                        env,
                        new FileCopyTaskInputFormat(tasks),
                        new GenericTypeInfo<>(edu.bear.flink.examples.java.distcp.FileCopyTask.class),
                        "fileCopyTasks");

        FlatMapOperator<edu.bear.flink.examples.java.distcp.FileCopyTask, Object> res =
                inputTasks.flatMap(
                        new RichFlatMapFunction<edu.bear.flink.examples.java.distcp.FileCopyTask, Object>() {
                            ...
                            @Override
                            public void flatMap(edu.bear.flink.examples.java.distcp.FileCopyTask task, Collector<Object> out)
                                    throws Exception {
    ...
    private static List<edu.bear.flink.examples.java.distcp.FileCopyTask> getCopyTasks(Path sourcePath) throws IOException {
        List<edu.bear.flink.examples.java.distcp.FileCopyTask> tasks = new ArrayList<>();
        getCopyTasks(sourcePath, "", tasks);
        return tasks;
    }

    private static void getCopyTasks(Path p, String rel, List<edu.bear.flink.examples.java.distcp.FileCopyTask> tasks)
            throws IOException {
        ...
        for (FileStatus fs : res) {
            if (fs.isDir()) {
                ...
                Path cp = fs.getPath();
                tasks.add(new edu.bear.flink.examples.java.distcp.FileCopyTask(cp, rel + cp.getName()));
            }
        }
    }
}
